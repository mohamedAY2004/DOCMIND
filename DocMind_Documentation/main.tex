...
\titleformat{\paragraph}[hang]
  {\fontsize{12}{14}\selectfont\underline}
  {}{0em}{}
...
\begin{titlepage}
  ...
  {\fontsize{14}{18}\selectfont\itshape Dr.\ Ahmed Saied\par}
...
The implemented system provides role-based access control (RBAC) for students, instructors, and administrators enforced in the Application Programming Interface (API) and web client, JSON Web Token (JWT)-based authentication with configurable token lifetime, administrator-controlled student-access gating, and validated document uploads with per-conversation limits. The backend is built on FastAPI with a PostgreSQL relational database, while semantic retrieval is powered by a pluggable vector store supporting both pgvector and Qdrant. The React-based frontend delivers a responsive Single-Page Application (SPA) accessible across desktop and mobile browsers. This document presents a comprehensive overview of the system, covering the literature review, system analysis and design, and implementation details, demonstrating the feasibility of integrating trustworthy AI-driven assistance into formal academic environments.

\clearpage

% ─── Abstract (Arabic) ───────────────────────────────────────────────────────
\chapter*{الملخص}
\addcontentsline{toc}{chapter}{الملخص (Arabic Abstract)}
\begin{Arabic}
\setlength{\parindent}{1.5em}
\setlength{\parskip}{6pt}

 DocMind هو مساعد تعليمي أكاديمي مدعوم بالذكاء الاصطناعي، صُمِّم لتعزيز تجربة التعلم لدى طلاب الجامعات من خلال توفير تفاعلات محادثية موثوقة ومرتكزة على المستندات. يعالج النظام التحديات الشائعة التي يواجهها الطلاب عند التعامل مع كميات كبيرة من المواد الأكاديمية، إذ يتيح لهم الاستعلام عن المحتوى الموثوق باستخدام اللغة الطبيعية.

على خلاف أنظمة الدردشة بالذكاء الاصطناعي ذات الأغراض العامة، يعتمد دوك مايند على تقنية التوليد المعزز بالاسترجاع (GAR) بحيث ترتكز الإجابات أساساً على مجموعة محتوى محددة: المواد التي يرفعها أعضاء هيئة التدريس لمحادثات المساعد الافتراضي (الدردشة بالمادة)، والملفات التي يرفعها الطلاب أنفسهم في محادثات المستندات. يدعم النظام وضعَي تفاعل رئيسيَّين: الدردشة المرتكزة على المستندات، والدردشة المرتكزة على مواد المادة الدراسية.

يوفر النظام المُنفَّذ تحكماً في الوصول قائماً على الأدوار للطلاب والمدرسين والمسؤولين، ومصادقة مبنية على رموز JWT مع مدة صلاحية قابلة للضبط، وبوابة للتحكم في وصول الطلاب يديرها المسؤول، فضلاً عن رفع مستندات محقَّق منها مع حدود لكل محادثة. تم بناء الواجهة الخلفية باستخدام FastAPI مع قاعدة بيانات علائقية PostgreSQL، فيما يعتمد الاسترجاع الدلالي على مخزن متجهات قابل للتوصيل يدعم كلاً من pgvector وQdrant. تقدم الواجهة الأمامية المبنية بـReact تطبيقاً من صفحة واحدة يعمل على أجهزة سطح المكتب والهاتف المحمول.
...
\begin{center}
  ...
  \textit{To our supervisor, Dr.\ Ahmed Saied, for his invaluable guidance.}\\[6pt]
...
Finally, we acknowledge the open-source communities behind FastAPI, React, PostgreSQL, LangChain, and ChromaDB, whose contributions to the software ecosystem made the technical realization of DocMind possible.
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Paper 1: Comparing Retrieval-Augmentation and Parameter Efficient Fine-Tuning for Privacy-Preserving Personalization of Large Language Models [1]}

\textbf{Authors}: Hamed Zamani, Alireza Salemi\\
...
The following figure compares the methods described --- RAG, PEFT, and PEFT-RAG. As shown in Figure~\ref{fig:lamp_comparison}, the combination of both techniques consistently yields the highest personalization scores across the LaMP benchmark tasks.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{lamp_comparison.png}
\caption{Comparison of Personalization Techniques Across LaMP Datasets}
\label{fig:lamp_comparison}
\end{figure}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Paper 2: A Comparison of Independent and Joint Fine-Tuning Strategies for Retrieval-Augmented Generation [2]}

\textbf{Authors}: Neal Lawton, Alfy Samuel, Anoop Kumar, Daben Liu\\
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Paper 3: Fine-Tuning or Fine-Failing? Debunking Performance Myths in Large Language Models [3]}

\textbf{Authors}: Scott Barnett, Zac Brannelly, Stefanus, Sheng Wong\\
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Paper 4: Enhancing Retrieval-Augmented Generation: A Study of Best Practices [4]}

\textbf{Authors}: Siran Li, Linus Stenzel, Carsten Eickhoff, Seyed Ali\\
...
\begin{itemize}[noitemsep]
  \item Uses FAISS as described in [15] with Sentence Transformers for semantic search.
...
The system depends on several external components and services, including embedding and text generation APIs. The codebase supports configurable providers such as OpenAI and Google Gemini, selected via environment settings. Additional dependencies include PostgreSQL and a vector index (pgvector or Qdrant). An optional agent layer, disabled by default, can route each user turn through a planner that decides whether to retrieve from the vector store before generation.
...
Another key strength is a clear separation into API, services, and data layers with role-based access control (RBAC) suitable for institutional deployment. The responsive web client increases accessibility across devices without maintaining separate native application codebases.
...
\begin{itemize}
  \item All API traffic shall use Transport Layer Security (TLS) in production deployments; passwords shall be stored using strong one-way hashing (bcrypt in the implementation), in line with established web security guidelines as described in [11].
  \item The system shall prevent unauthorized access to academic content through role checks on every protected route.
  \item The system shall be designed so that institutional data privacy policies can be applied at deployment time (retention, backups, and optional at-rest database encryption are operator responsibilities), in accordance with applicable data protection regulations as described in [12].
\end{itemize}
...
\begin{itemize}
  ...
  \item Chat interface for question submission, streaming or polled assistant replies, and optional thumbs-up/down feedback
...
\begin{itemize}
  ...
  \item Subject, semester, and material management (SubjectService, MaterialService)
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{system_block_diagram.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{sequence_diagram_rag.png}
...
Each entity in the ERD shown in Figure~\ref{fig:erd} corresponds to a logical system component. Primary and foreign keys enforce relationships between users, subjects, documents, sessions, and embeddings.
...
\begin{itemize}
  \item \textbf{Backend}: Python 3.11+, FastAPI as described in [13], SQLAlchemy (async), Alembic for migrations, Pydantic for data validation.
  \item \textbf{AI/ML}: LangChain as described in [14] for RAG orchestration, Google Gemini API (configurable to OpenAI), and \texttt{sentence-transformers} (all-MiniLM-L6-v2) for local embeddings.
  \item \textbf{Vector Store}: pgvector as described in [16] (PostgreSQL extension) or Qdrant as described in [17], switchable by environment configuration.
  \item \textbf{Relational Database}: PostgreSQL 15+.
  \item \textbf{Frontend}: React 18 with Vite, Tailwind CSS, React Router, and Axios for API calls.
  ...
  \item \textbf{Version Control}: Git with GitHub, using a submodule-based monorepo structure.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Routers} (\texttt{/api/v1/...}): Handle HTTP request routing and response serialization.
...
\begin{enumerate}
  ...
  \item Triggers an asynchronous ingestion task that reads the PDF, splits it into configurable-size chunks using LangChain text splitters, generates embeddings for each chunk, and upserts the vectors into the conversation-scoped collection in the vector store.
...
\begin{enumerate}
  \item The backend retrieves the top-$k$ most semantically similar chunks from the conversation or subject collection.
  \item Chunk texts are assembled into a context string, which is inserted into a prompt template available in English and Arabic locales.
  \item The prompt is sent to the configured LLM API (Gemini or OpenAI) for generation.
  \item The response is stored in the \texttt{messages} table, associated with the conversation, and returned to the client.
  \item Optionally, if the agentic mode is enabled, a planner model first decides whether retrieval is needed and with what reformulated query before the generator runs.
\end{enumerate}

\subsection{Administrative APIs}

The admin router exposes endpoints for user Create, Read, Update, and Delete (CRUD) operations (create instructor accounts, activate or deactivate users, reset passwords), subject and semester management, aggregated statistics (daily message counts, subject-level activity, per-user usage), feedback records collected from student thumbs-up/down interactions, a global student-access flag that gates all student-facing routes with a configurable maintenance message, and a structured activity log recording every administrative action for audit review.
...
\begin{itemize}
  ...
  \item \textbf{Administrator}: User management, subject management, analytics, feedback viewer, activity log, and system settings.
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{prototype_student_docbased.png}
...
The tutor (subject-based) chat interface allows students to select a course or subject for which they are enrolled and for which instructors have uploaded materials. Retrieval is limited to indexed content for that subject. The interface surfaces the active subject and instructor context so that answers remain visually tied to the chosen course.

Figure~\ref{fig:student_subject} shows the student interface for subject-based chat.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{prototype_student_subject.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{prototype_instructor_content.png}
...
\begin{itemize}
  ...
  \item \textbf{Integration Testing}: API endpoints are tested end-to-end using FastAPI's TestClient, exercising the full request-response cycle including authentication, authorization, and database interaction against a test database instance.
...
The current RAG pipeline uses a single-stage dense retrieval approach. Future improvements, informed by best-practice findings in [4], include hybrid retrieval combining dense vector search with keyword-based (BM25) sparse retrieval, re-ranking models (cross-encoders) to improve precision of the top-$k$ retrieved chunks, and query expansion using lightweight models to improve recall for short or ambiguous student queries.
...
The current codebase targets responsive web browsers. A dedicated mobile application for iOS and Android would improve student accessibility, particularly for reviewing materials on the go. The existing FastAPI backend is already designed as a platform-agnostic API, making a mobile client straightforward to develop.
...
\begin{list}{[\arabic{enumi}]}{\usecounter{enumi}\setlength{\leftmargin}{2.5em}\setlength{\itemsep}{6pt}}
  ...
  \item IEEE Std 1016-2009, \textit{IEEE Standard for Information Technology --- Systems Design --- Software Design Descriptions}, IEEE Computer Society, 2009.

  \item OWASP Foundation, \textit{OWASP Top Ten Web Application Security Risks}, 2023. [Online]. Available: \url{https://owasp.org/www-project-top-ten/}

  \item European Union, ``General Data Protection Regulation (GDPR),'' Regulation (EU) 2016/679, Off.\ J.\ Eur.\ Union, Apr.\ 2016.

  \item S.\ Tiangolo, \textit{FastAPI: Modern, Fast Web Framework for Building APIs with Python}. [Online]. Available: \url{https://fastapi.tiangolo.com}

  \item J.\ Harrison, \textit{LangChain: Building Applications with LLMs}. [Online]. Available: \url{https://python.langchain.com}

  \item A.\ Johnson et al., ``FAISS: A Library for Efficient Similarity Search,'' Facebook AI Research. [Online]. Available: \url{https://github.com/facebookresearch/faiss}

  \item pgvector Contributors, \textit{pgvector: Open-Source Vector Similarity Search for PostgreSQL}. [Online]. Available: \url{https://github.com/pgvector/pgvector}

  \item Qdrant Team, \textit{Qdrant Vector Database Documentation}. [Online]. Available: \url{https://qdrant.tech/documentation/}

  \item Google, \textit{NotebookLM: AI-Powered Research and Learning Tool}. [Online]. Available: \url{https://notebooklm.google.com}

  \item Humata AI, \textit{Humata: AI-Powered Document Assistant}. [Online]. Available: \url{https://www.humata.ai}

  \item Mindgrasp, \textit{Mindgrasp: AI Learning and Study Platform}. [Online]. Available: \url{https://mindgrasp.ai}

  \item AskYourPDF, \textit{AskYourPDF: Chat with Your Documents}. [Online]. Available: \url{https://askyourpdf.com}
