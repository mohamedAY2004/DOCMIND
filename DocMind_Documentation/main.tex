...
\begin{itemize}
  ...
  \item \textbf{AI/ML}: an in-house Retrieval-Augmented Generation (RAG) orchestration layer (structure-aware chunking, prompt assembly, and retrieval), building on patterns popularized by libraries such as LangChain as described in [14], Google Gemini API (configurable to OpenAI), \texttt{sentence-transformers} (all-MiniLM-L6-v2) for local embeddings, and an optional \texttt{sentence-transformers} cross-encoder for reranking.
